...
import { getAllTrips } from "./apiservice";
...
const User = () => {
  ...
  const [selectedOptionPrice, setSelectedPrice] = useState("");
  const [selectedOptionDuration, setSelectedDuration] = useState("");
            ...
            {tripData?.map((trip) => {
              return (
                <tr className="bg-white border-b " key={trip?._id}>
                  ...
                  {/* <td className="py-4 px-2">
                    {trip?.user?.country ? trip?.user?.country : "-"}
                  </td> */}
